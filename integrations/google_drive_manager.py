...
class GoogleDriveManager:
    ...
    def upload_to_drive(self, file_buffer: io.BytesIO, filename: str) -> bool:
        ...
        try:
            # Get owner email from folder to transfer ownership
            owner_email = self._get_folder_owner_email()
            ...
            media = MediaIoBaseUpload(
                ...
                resumable=False  # Use non-resumable for smaller files
            )

            # Create file without supportsAllDrives first
            try:
            ...
            file_id = file.get('id')
            if file_id:
                # Transfer ownership to personal account if we have owner email
                if owner_email:
                    try:
                        ...
                        print(f"⚠️ Could not transfer ownership: {perm_error}")
                
                # Set anyone can view permissions
                try:
                    self.service.permissions().create(
                        fileId=file_id,
    ...
    def _get_folder_owner_email(self):
        """Get the owner email of the shared folder"""
        try:
            if not self.folder_id:
                return None
                
            # Get folder permissions to find owner
            ...
            for perm in permissions.get('permissions', []):
                if perm.get('role') == 'owner':
                    ...
                    if owner_email and not owner_email.endswith('.iam.gserviceaccount.com'):
                        print(f"📧 Found folder owner: {owner_email}")
                        return owner_email
            
            return None
        except Exception as e:
            print(f"⚠️ Could not get folder owner: {e}")
            return None
